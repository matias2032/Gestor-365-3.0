...
Future<void> _adicionarAoPedido() async {
  ...
  try {
    final idUsuario = 1; // SUBSTITUIR PELO ID DO USUÁRIO LOGADO
    
    // Criar item
    final item = ItemPedido(
      idPedido: 0, // Será definido pelo BD
    ...
    if (_pedidoAtivoService.temPedidoAtivo) {
      ...
      // 🔥 CORREÇÃO: Usar DatabaseService diretamente
      final db = await _dbService.database;
      
      await db.transaction((txn) async {
        // Inserir item
        await txn.insert('item_pedido', item.copyWith(idPedido: pedidoId).toMap());
        
        // Debitar estoque
        await txn.rawUpdate(
          'UPDATE produto SET quantidade_estoque = quantidade_estoque - ? WHERE id_produto = ?',
          [_quantidade, _produto!.id],
        );
      ...
      // 🔥 USAR O MÉTODO DO SYNC SERVICE (já atualizado)
