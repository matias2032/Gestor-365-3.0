...
class _DetalhesUsuarioScreenState extends State<DetalhesUsuarioScreen> {
  ...
  Future<void> _reiniciarSenha(Usuario usuario) async {
    ...
    try {
      // 1. Gerar hash da senha padrão: 12345678
      const senhaPadrao = '12345678';
      final senhaHash = await FlutterBcrypt.hashPw(
        password: senhaPadrao,
        salt: await FlutterBcrypt.salt(),
      );

      // 2. Atualizar usuário no banco
      final usuarioAtualizado = usuario.copyWith(
        senhaHash: senhaHash,
        primeiraSenha: 1, // 🔥 Força troca de senha no próximo login
      );

     await _dbService.updateUsuario(usuarioAtualizado);
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              '✅ Senha de ${usuario.nome} foi reiniciada para 12345678\n'
              'No próximo login, será obrigado a criar uma nova senha.',
            ),
