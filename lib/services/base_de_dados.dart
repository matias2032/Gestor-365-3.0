...
Future<int> createPedido(Pedido pedido, List<ItemPedido> itens) async {
  ...
  return await db.transaction((txn) async {
    ...
    // 1.2. Criar o pedido (usa txn)
    final idPedido = await txn.insert('pedido', pedido.toMap());

    // 1.3. Inserir itens e debitar estoque (tudo com txn)
    for (final item in itens) {
    ...
    // Opcional: recalcular total do pedido aqui (por segurança), utilizando somatório dos itens inseridos
    ...
    await txn.update('pedido', {'total': total}, where: 'id_pedido = ?', whereArgs: [idPedido]);
...
Future<void> updateQuantidadeItem(int idItemPedido, int novaQuantidade) async {
  ...
  await db.transaction((txn) async {
    ...
    // Atualizar estoque (se diminuir quantidade, credita; se aumentar, debita)
    if (diferencaQuantidade != 0) {
      await txn.rawUpdate(
        'UPDATE produto SET quantidade_estoque = quantidade_estoque - ? WHERE id_produto = ?',
